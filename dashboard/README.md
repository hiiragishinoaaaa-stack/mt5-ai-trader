...
- ✅ Settings画面上部「AI判断ロジック」「発注設定」「リスク管理」
  ...
  balanced/aggressive/active_m5)は必須条件+加点スコアリング方式のAI判断
  ロジックを切り替えるプリセットで、選択すると関連する数値がまとめて
  更新される(balancedを含め、旧来の単純な3条件AND判定からは変更されて
  いる。詳細は`mt5_ai_trader/README.md`のPhase 13参照)。日次サマリーは1日1回、その日の
