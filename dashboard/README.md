...
OpenAI/Claude連携(Phase 7)」「決済通知(Phase 8)」「同時ポジション数の上限
(Phase 9)」「Dashboardからの手動決済(Phase 10)」を参照)。VPS/MT5参考情報
はまだUIモック。
...
- ✅ Analytics: 累積損益・日別/月別損益・勝率・プロフィットファクター・
  ...
  取得範囲=既定30日/最大50件に限る)
- ✅ Settings画面上部「AI判断ロジック」「発注設定」「リスク管理」
  「Discord通知」: `settings_server.py`経由でPython側の実際の設定を
  取得・保存できる(ORDER_VOLUME/SL_POINTS/TP_POINTS/STOP_MODE/
  ATR_SL_MULTIPLIER/ATR_TP_MULTIPLIER/POINT_SIZE/BROKER_MIN_STOP_POINTS/
  TIMEFRAME/LOOP_INTERVAL_SECONDS/RSI_BUY_MIN/RSI_BUY_MAX/RSI_SELL_MIN/
  RSI_SELL_MAX/RSI_PERIOD/ATR_PERIOD/REQUIRED_SCORE/MAX_SPREAD_POINTS/
  ATR_MIN_POINTS/EMA_FAST_PERIOD/EMA_SLOW_PERIOD/Entry Strictness/
  ENTRY_COOLDOWN_SECONDS/MAX_TRADES_PER_HOUR/MAX_TRADES_PER_DAY/
  LOSS_STREAK_THRESHOLD/COOLDOWN_AFTER_LOSSES_MINUTES/
  MAX_DAILY_LOSS_PERCENT/ENABLE_ORDERS/DEMO_ONLY/DISCORD_ENABLED/
  DISCORD_WEBHOOK_URL/DISCORD_NOTIFY_ON_TRADE/DISCORD_NOTIFY_ON_ERROR/
  DISCORD_NOTIFY_DAILY_SUMMARY/AI_ENGINE)。Entry Strictness(conservative/
  balanced/aggressive/active_m5)は統一スコア閾値方式のAI判断ロジックを
  切り替えるプリセットで、選択すると関連する数値がまとめて更新される
  (判断条件を1点ずつ均等に採点し、合計がREQUIRED_SCORE以上でエントリー。
  詳細は`mt5_ai_trader/README.md`のPhase 13参照)。日次サマリーは1日1回、その日の
  損益をDiscordへ送信する(詳細は`mt5_ai_trader/README.md`のPhase 6参照)。
  AI_ENGINE(rule_based/openai/claude/gemini)を切り替えると、実際にOpenAI/
  Claude/GeminiのAPIを呼び出してBUY/SELL/WAITを判断させられる(APIキー自体は
  セキュリティ上の理由でDashboardには出さず`.env`でのみ設定する。詳細は
  Phase 7参照)。
...
- ✅ Trade: ポジションカードのCLOSEボタン。ARTEMIS自身が保有するポジション
  をPCなしでその場で決済できる(要EA v4.03以降。詳細はPhase 10参照)
- ✅ Settings: TIMEFRAMEの変更は、EA v4.04以降ならPCなしで次サイクルから
  実際のローソク足取得時間軸にも自動反映される(v4.03以前はPython側の
  ラベル表示のみに反映され、実際の時間軸を変えるにはMT5側のEA入力
  パラメータを手動で変更する必要がある。詳細は`mt5_ai_trader/README.md`の
  Phase 11参照)
- ✅ Settings: 「銘柄」セクションでUSDJPY以外(現時点ではEURUSD)をON/OFF
  できる。ONの銘柄だけがAI判断・発注の対象になり、Trade画面のAI Judgement
  にも銘柄ごとにカードが表示される。ONにするには、MT5側にその銘柄用の
  EAインスタンスを別チャートへ追加する必要がある(入力パラメータを変える
  だけで再コンパイルは不要。詳細は`mt5_ai_trader/README.md`のPhase 12参照)
