...
## AI判断の履歴ログ(Phase 16.1)

`ai_status.write_status()`(Dashboard表示用)は最新1件だけを毎サイクル
上書きするため、`REQUIRED_SCORE`をどの値にすべきかを後から実測で検証
する材料が残らなかった(バックテスト基盤が無い分、この履歴が唯一の
実データになる。2026-07、Fable5との相談を踏まえて追加)。

`ai_status.append_decision_log()`が毎サイクル、`AI_DECISION_LOG_PATH`
(既定`mt5_ai_trader/logs/ai_decisions.jsonl`、複数銘柄対応のため銘柄ごとに
別ファイル)へ1行1JSON(JSON Lines)で追記する。`ai_engine.RuleBasedAIEngine.
decide()`は2026-07から、実際に発動した方向だけでなくBUY/SELL両方向の
スコア内訳(`buy_score`/`buy_total`/`buy_failed`/`sell_score`/`sell_total`/
`sell_failed`)を常に`Signal.details`へ含めるようになっており、この履歴には
そのまま両方向の内訳が残る。これにより、後から「`REQUIRED_SCORE`をNに
していたらこの足はどう判断されていたか」をログを読むだけで再現できる
(RuleBasedAIEngine以外のエンジンでは対応するキーが無いため`null`になる)。

得点内訳だけでなく、その足の生の指標値(`open`/`close`/`high`/`low`/
`ema_fast`/`ema_slow`/`rsi`/`macd_hist`/`atr`/`spread`/`h1_ema_fast`等)も
一緒に記録している。スコアだけだと「RSI帯域を変えたらどうなっていたか」の
ような条件そのものの見直しは過去ログから再現できないため、生値もあれば
再稼働なしで再採点できる(安い投資、2026-07、Fable5との相談を踏まえて追加)。

書き込み失敗(ディスク容量不足等)はログに残すだけで判断・発注フロー自体は
止めない(既存の`write_status`と同じ設計方針)。

`ai_decisions.jsonl`/`ai_shadow_log.jsonl`(Geminiシャドーモード参照)は
どちらも無制限に追記され続けるため、`AI_LOG_MAX_BYTES`(既定20MB)を
超えたら`ai_status._rotate_if_needed()`が世代交代ローテーションする
(`<file>.1.jsonl` → `.2.jsonl` → ...、`AI_LOG_BACKUP_COUNT`(既定5)世代を
超えた最古の世代は削除)。VPSのディスクを無制限に食い潰さないための対策
(2026-07、Fable5との相談を踏まえて追加)。

## Geminiシャドーモード(Phase 16.2)

`AI_ENGINE`を切り替えると判断ロジックが丸ごと入れ替わるため、その後成績が
変わっても「スコア制(Phase 15)のおかげ/せい」なのか「Geminiのおかげ/せい」
なのか区別できなくなる(変更は1回に1つが検証の鉄則。2026-07、Fable5との
相談を踏まえて追加)。

`GEMINI_SHADOW=true`にすると、実際の発注判断は`AI_ENGINE`(既定
`rule_based`)のまま一切変えず、同じ入力に対してGeminiにも並行して
BUY/SELL/WAITを判定させ、結果は発注に一切使わずに記録だけする
(`ai_engine.get_shadow_engine()`、`AI_ENGINE=gemini`のCandleThrottled
Engineと全く同じインスタンスをシャドー用に使うため、コスト対策も同じ
まま無料枠に収まる想定)。

- **ログ**: `ai_status.append_shadow_log()`が毎サイクル、
  `AI_SHADOW_LOG_PATH`(既定`mt5_ai_trader/logs/ai_shadow_log.jsonl`、
  複数銘柄対応のため銘柄ごとに別ファイル)へ`rule_action`/`gemini_action`/
  `gemini_confidence`/`agree`(一致したか)/`price`(判断時点のclose、後から
  値動きと突き合わせて仮想損益を計算するための参考値)を1行1JSON
  (JSON Lines)で追記する。
- **Dashboard**: `ai_status.write_status()`が最新の判断に`gemini_shadow`
  (action/confidence/reason)を含めるようになり、Trade画面のAI Judgement
  カードに実際の判断と並べて「Gemini(shadow): BUY 82%」のように表示される
  (`RealAiStatus.gemini_shadow`参照。有効化はDashboard Settingsの
  「AI判断ロジック」内`Gemini Shadow Mode`トグルから)。
- シャドー判断でエラーが起きても(APIキー未設定・タイムアウト等)、記録が
  欠けるだけで本来のAI判断・発注フローには一切影響しない。
- 一致率・Geminiに従っていた場合の仮想損益の集計は`gemini_shadow_report.py`
  (Phase 16.4参照)で行う。

## ヒストリカル・リプレイのバックテスト(Phase 16.3)

`ai_decisions.jsonl`(Phase 16.1)は「今日から先」しか貯まらないため、
`REQUIRED_SCORE`をどの値にすべきかを統計的に比較できる量が貯まるまで
M15×少数銘柄だと数週間〜数ヶ月かかる。`RuleBasedAIEngine`のスコアロジックを
**過去のローソク足に対してオフラインで再生**すれば、その期間分を今日
手に入れられる(2026-07、Fable5との相談を踏まえて追加)。

### 1. 過去足のエクスポート(MT5側、1回限り)

Python側はMT5 APIを直接使わない設計(EAが書き出すJSONファイルをPythonが
読むEA-bridgeアーキテクチャ)のため、過去足は`ea/ARTEMIS_HistoryExport.mq5`
という**スクリプト**
(常駐する`ARTEMIS_Bridge.mq5`とは別、チャートへ1回ドラッグして実行する
使い切りのスクリプト。EAの動作には一切影響しない)でエクスポートする。

1. MetaEditorで`ARTEMIS_HistoryExport.mq5`を開いてコンパイル
2. MT5のナビゲーターの「スクリプト」から、対象銘柄・時間足のチャートへ
   ドラッグ(`InpSymbol`/`InpTimeframe`/`InpBarsCount`を入力ダイアログで
   調整できる。既定5000本≒M15で約52日分。実際に取得できる本数はブローカーの
   サーバー側の履歴保持期間次第で、要求本数より少なくなることがある。
   「エキスパート」タブのログで実際にエクスポートされた本数を確認できる)
3. `%APPDATA%\MetaQuotes\Terminal\Common\Files\artemis_history_<銘柄>_<時間足>.json`
   が書き出される(`ARTEMIS_Bridge.mq5`の稼働状況とは無関係)
4. そのファイルを`backtest_replay.py`を実行する環境(VPS等)へコピーする
...
- 各バーの判断は、**本番(main.py)と全く同じローリングウィンドウ方式**
  (直近`BARS_COUNT`本、既定100本)で指標を計算し直してから
  `RuleBasedAIEngine.decide()`を呼ぶ(全履歴に対して指標を1回だけ計算して
  スライスする方式だと、EMA/RSI等が本番と異なる値になってしまうため)。
- 決済判定は`SL_POINTS`/`TP_POINTS`固定幅・先着方式(`STOP_MODE=atr`の
  動的SL/TPは未対応)。1本のローソク足内でSL/TP両方に価格が触れた場合、
  実際にどちらが先だったかはローソク足データだけでは分からないため、
  既定では保守的にSLを優先する(`--optimistic-fill`でTP優先に変更可)。
- `risk_manager.py`(クールダウン・同時保有数上限・連敗停止等)は再現して
  いない。このツールが検証するのはあくまでRuleBasedAIEngineのスコア
  ロジック単体の期待値であり、本番の発注頻度そのものではない。
- `--decision-log-out`を指定すると、全バーの判断内訳を`ai_decisions.jsonl`
  と同じスキーマでJSON Linesへ書き出せる(実運用ログと突合できる)。

## Geminiシャドーモードの集計(Phase 16.4)

`gemini_shadow_report.py`が、`ai_shadow_log.jsonl`(Phase 16.2)を集計する:

```
# 一致率だけ(ローソク足データ不要)
.venv/bin/python gemini_shadow_report.py --shadow-log logs/ai_shadow_log.jsonl

# 一致率+仮想損益(Phase 16.3のARTEMIS_HistoryExport.mq5で取得した過去足が必要)
.venv/bin/python gemini_shadow_report.py --shadow-log logs/ai_shadow_log.jsonl \
    --candles-file artemis_history_USDJPY_M15.json
```

- **一致率**: ルール判断とGemini判断が一致した割合(全体、および
  ルールがBUY/SELL/WAITだったときそれぞれの内訳)。ログの`agree`列を
  集計するだけなので、過去足データが無くても計算できる。
- **仮想損益**: Geminiの各BUY/SELL判断について、判断時点の価格から
  `backtest_replay.py`と同じSL/TP先着判定(`simulate_trade_forward`、
  同じ関数を共有)で決済まで進める。比較のため、同じ方式で「ルールの
  判断に従っていた場合」の仮想損益も併せて計算する(両者を同一の
  シミュレーション方式で比較するのが目的で、実際の約定[スリッページ等]
  とは異なる参考値)。
...
### USDJPY M15・52日での初回結果(2026-07)

上記の監査を実際に回した結果、**採点条件がエッジと真逆を向いている**ことが
判明した:

- **単調性**: RSIが低い分位(売られすぎ)でBUY勝率44.9%、RSI帯域内が該当する
  中間分位(50〜61)は29〜31%(最低)。EMA乖離も、価格がEMAより下(下乖離)で
  BUY勝率43.5%。つまりエッジは「売られすぎを買う(平均回帰)」にある。
- ところが現行の条件(`RSI帯域内`=50〜65、`押し目からの回復`=EMA近辺への
  復帰)は、勝率が最も低いゾーンをピンポイントで選んでいる。順張り系条件を
  AND合成するほどこの最悪ゾーンに集中し、単一条件(MACD方向一致32.9%)より
  全条件合成(28%)の方が悪化する。
- **レジーム依存**: トレンド相場(ADX≥25)ではMACD方向一致が損益分岐超え・
  ATR全分位プラスと機能するが、レンジ相場(ADX<25、全体の63%)は完全な
  平均回帰(RSI・EMA乖離とも単調、低ボラで高勝率)。戦略が負けるのは、
  レンジ多数派を順張りロジックで取引しているため。

### 期間別の"方向の保存"検証(`--by-period`、Phase 16.5.1)

上記の派手な数字(44.9%等)は「この52日から発見した数字」なので、それを
根拠に設計して同じ期間で検証するのは無意味(答えを見て答案を書く行為)。
`--by-period`は、過去2〜3年+複数銘柄へ拡張したデータに対し、**単調性の
"方向"(RSIが低い側で勝つ、等)が期間をまたいで保存されるか**を検証する
(Fable5の指示。判定基準は数字の大きさではなく符号が反転しないこと)。

```
.venv/bin/python backtest_audit.py --candles-file artemis_history_USDJPY_M15.json \
    --by-period --spread-points 15
```

出力:
- **期間別の概況**: 四半期ごとのバー数・ADX<25(レンジ)比率・レジーム別の
  ベースライン勝率。相場環境の期間推移が分かる。
- **単調性の方向テーブル**: 指標(RSI/EMA乖離/ATR/MACD)×四半期で、各分位
  分析の`Q1勝率 − Q5勝率`(符号付き)を並べる。**符号が期間で反転しなければ
  そのエッジは保存されている(本物の候補)、反転すればその期間固有のまぐれ。**
- 直近`--discovery-days`(既定52)日は「エッジ発見に使った期間」として
  `[発見]`列に分離し、四半期側(≒アウトオブサンプル)から除く。

`--by-period`は条件別成否を計算しない軽量版(全系列を1度だけ指標計算)に
切り替わるため、2〜3年規模でも実用的な速度で終わる(EMA/RSI等は系列先頭
付近で本番のローリングウィンドウ版と厳密には一致しないが、単調性の"方向"を
見る用途では影響は無視できる)。過去足のエクスポート方法はPhase 16.3参照
(`ARTEMIS_HistoryExport.mq5`の`InpBarsCount`を増やし、EURUSDのチャートにも
実行する)。
