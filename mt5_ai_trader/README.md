...
mt5_ai_trader/
  ...
  mt5_client.py   MT5ターミナルとの通信(接続〜ティック/ローソク足取得を
                  別プロセスで実行し、ハング時はterminateできるようにする)。
...
`mt5.initialize()` 呼び出しの前後にログを出力しているため、`--debug` を
付けて実行し、`logs/trades.log` またはコンソール出力で以下を確認する。

- `MT5_PATH` が正しいファイルに存在しているか(ログに `存在=True/False` と出る)
- `login` / `server` が空欄になっていないか
- MT5ターミナルが起動・デモ口座にログイン済みか

`MetaTrader5`パッケージのC拡張は、IPC待機中にブロックした場合にPythonの
GIL(Global Interpreter Lock)を解放しないことがあり、その場合は別スレッド
によるタイムアウトでは検知・打ち切りができない。そのため接続〜データ取得は
常に別プロセス(`multiprocessing`)で実行し、`MT5_FETCH_TIMEOUT_SECONDS`
(既定20秒、`.env`で変更可)を超えた場合はプロセスごと強制終了
(`terminate()`/`kill()`)する。これによりC拡張側の状態に関わらず、
`main.py --once` は必ず一定時間内に成功またはエラーで終了する。

それでもタイムアウトが続く場合は、MT5側のIPC自体が応答していない
(ターミナルが実際にはフリーズしている、ネットワーク遮断、アルゴ取引が
無効になっている等)可能性が高いため、MT5ターミナルの再起動や
「アルゴ取引を許可する」設定を確認すること。
