...
- `config.BOT_RUN_STATE` がRUNNINGでない限り(DashboardのSTOP/EMERGENCY
  STOP)、発注リクエストは一切書き出さない。
- `config.DEMO_ONLY` が明示的にTrueでない限り、発注リクエストは一切
  書き出さない(既定でOFF)。
- 実際にデモ口座かどうかの最終確認、既存ポジション数のチェック(発注
  リクエストに含める`max_positions`=`config.MAX_CONCURRENT_POSITIONS`を
  上限として、実際のカウント・強制はEA側が行う)、発注の実行そのものは
  全てEA側の責任とする。Python側はMT5の口座状態を一切知り得ない
  (EAブリッジ方式の制約)ため、「Pythonだけを信用しない」設計にしている。
"""
...
class FileOrderExecutor:
    ...
    def submit_if_needed(
        ...
        if config.BOT_RUN_STATE != "RUNNING":
            # 通常はmain.py run_once()がBOT_RUN_STATE!=RUNNINGの時点でここに
            # 到達しないが、直接呼び出された場合に備えた多層防御。
            logger.info(
                "order_executor: BOT_RUN_STATE=%sのため発注をスキップします"
                "(DashboardのSTARTボタンで再開できます)",
                config.BOT_RUN_STATE,
            )
            return None
