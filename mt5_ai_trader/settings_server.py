...
## エンドポイント
    ...
    GET  /api/ai-status    main.pyが各サイクルで書き出す最新のAI判断
                         ...
                         クエリパラメータ?symbol=EURUSD等で対象銘柄を指定
                         できる(複数銘柄対応、Phase 12)。省略時はconfig.SYMBOL
                         (プライマリ銘柄)。
    GET  /api/trade-history EA(ARTEMIS_Bridge.mq5、v4.00以降)が書き出す
                         決済済み取引一覧を返す(trade_history_feed.pyを
                         参照)。MT5/EA側がまだデータを書き出していない
                         場合は503を返す。銘柄を問わず全て含まれる
                         (EA側が既に全銘柄分を書き出しているため)。
    POST /api/close-position DashboardのCLOSEボタンから呼ばれる。
                         指定した銘柄のARTEMIS自身の全ポジションを決済
                         するようEAへ要求する(position_closer.pyを参照、
                         要EA v4.03以降)。対象銘柄はPOSTボディの
                         {"symbol": "EURUSD"}または?symbol=クエリで指定
                         (省略時はconfig.SYMBOL)。settings_schema.
                         AVAILABLE_SYMBOLSに無い銘柄は400を返す。
                         ENABLE_ORDERS/DEMO_ONLYが有効でない場合は決済
                         リクエストを送出せず失敗を返す。EAの応答を
                         待つため、最大ORDER_RESULT_WAIT_SECONDS秒
                         ブロックする。
...
class SettingsRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        elif path == _ACCOUNT_PATH:
            self._handle_get_account()
        elif path == _AI_STATUS_PATH:
            self._handle_get_ai_status()
        elif path == _TRADE_HISTORY_PATH:
    ...
    def _handle_get_ai_status(self) -> None:
        ...
        symbol = self._query_param("symbol") or config.SYMBOL
        try:
            snapshot = ai_status.read_status(symbol)
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        elif path == _CLOSE_POSITION_PATH:
    ...
    def _handle_post_close_position(self) -> None:
        ...
        symbol = payload.get("symbol") or self._query_param("symbol") or config.SYMBOL
        if symbol not in settings_schema.AVAILABLE_SYMBOLS:
            self._send_json(400, {"success": False, "message": f"不明な銘柄です: {symbol}"})
            return

        result = _position_closer.close_all(symbol)
