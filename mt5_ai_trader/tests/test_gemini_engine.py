"""gemini_engine.py の単体テスト。実際にGeminiへは接続しない
...
def _gemini_response(action: str, reason: str = "test", confidence: int = 70) -> dict:
    content = json.dumps({"action": action, "reason": reason, "confidence": confidence})
    return {"candidates": [{"content": {"parts": [{"text": content}], "role": "model"}}]}


def test_decide_returns_wait_when_api_key_missing(monkeypatch):
    monkeypatch.setattr(config, "GEMINI_API_KEY", "")
    engine = gemini_engine.GeminiEngine()

    signal = engine.decide(pd.DataFrame([_row()]))

    assert signal.action == "WAIT"
    assert "GEMINI_API_KEY" in signal.reason
...
def test_decide_parses_successful_buy_response(monkeypatch):
    ...
    def _fake_urlopen(req, timeout=None):
        calls.append(req)
        return _FakeResponse(_gemini_response("BUY", "上昇トレンド", 80))
    ...
    assert len(calls) == 1
    assert calls[0].full_url == "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent"
...
def test_decide_falls_back_to_wait_on_invalid_json_content(monkeypatch):
    def _fake_urlopen(req, timeout=None):
        return _FakeResponse(
            {"candidates": [{"content": {"parts": [{"text": "not json at all"}], "role": "model"}}]}
        )
